...
def segment_best_available(gray, bgr_for_color=None):
    """Use the trained U-Net if it's been trained (ml/shell_unet.pt exists);
    otherwise fall back to the classical Otsu-threshold segment(). Lets this
    script run before training finishes, and automatically upgrades once it
    has."""
    ckpt = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ml", "shell_unet.pt")
    if os.path.exists(ckpt):
        from ml.infer import segment_ml
        return segment_ml(gray, bgr_for_color=bgr_for_color)
    return segment(gray)
...
if __name__ == "__main__":
    ...
    mask    = segment_best_available(gray, bgr_for_color=img)
